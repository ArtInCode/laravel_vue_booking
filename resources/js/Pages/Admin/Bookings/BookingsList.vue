...
const editRoutLink = ref(route('bookings.edit', {id : 1}));
editRoutLink.value = editRoutLink.value.split('/').slice(0, editRoutLink.value.split('/').length - 1).join('/');
const deleteRoutLink = ref(route('bookings.delete', { id : 1}));
...
<template>
    ...
    <div class="m-auto w-1/2 py-12">
        <div>
          <Link :href="route('bookings.create')" class="rounded-md text-white bg-green-900 px-3 py-3">Create Booking Item</Link>
        </div>
        <div v-show="bookings.data.length" v-for="(item, index) in  bookings.data" :key="index" :id="`slider-${item.id}`" :class="[ index % 2 === 0 ? ' bg-emerald-700' : 'odd', 'grid grid-cols-6 gap-4 text-white align-middle place-items-center border border-white px-1 py-1 mt-5']">
        ...
        <div v-show="!bookings.data.length" class="text-white mt-5">
            No Slider is saved
        </div>
        
        <div v-if="bookings.data.length > 0" class="mt-5 text-white">
          <ModelPagination :links="bookings.links"></ModelPagination>
        </div>
    </div>
    </AuthenticatedLayout>
</template>